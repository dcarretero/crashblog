...
class Post(models.Model):
    ...
    tags = models.ManyToManyField('Tag', related_name='posts_set', blank=True)
...
class Tag(models.Model):
    name = models.CharField(max_length=100)
    slug = models.SlugField(max_length=100, unique=True)
    #posts = models.ManyToManyField(Post, related_name='tags_set',blank=True)

    def __str__(self):
        return self.name
    def get_absolute_url(self):
        return '/%s/%s/%s/' % ('blog','tag', self.slug)